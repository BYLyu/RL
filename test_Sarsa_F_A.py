...
# 计算衰减率
config['alpha_decay'] = (config['alpha_start'] - config['alpha_end']) / config['num_episodes']
config['epsilon_decay'] = (config['epsilon_start'] - config['epsilon_end']) / config['num_episodes']

# 创建 agent 时不再需要 feature_dim
agent = SarsaFA(env, config['epsilon_start'], config['alpha_start'], config['gamma'])
...
with tqdm(total=total_episodes, desc='Training Progress') as pbar:
    for i_episode in range(total_episodes):
        ...
        if (i_episode + 1) % 10 == 0:
            pbar.set_postfix({
                'episode': f'{i_episode + 1}/{total_episodes}',
